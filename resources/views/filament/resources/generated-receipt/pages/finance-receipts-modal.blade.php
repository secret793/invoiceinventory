<div class="p-6" x-data @open-export-url.window="window.open($event.detail.url, '_blank')">
    ...
    <div class="mb-6 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-900 dark:to-gray-800 p-4 rounded-lg border border-blue-200 dark:border-gray-700">
        ...
        <div class="flex flex-wrap gap-2 justify-end">
            ...
            <button type="button"
                    ...
                    class="px-4 py-2 text-sm font-medium text-white bg-green-600 hover:bg-green-700 dark:bg-green-700 dark:hover:bg-green-800 border border-green-600 rounded-md transition-all">
                📥 Export Excel
            </button>
            <button type="button"
                    wire:click.prevent="exportReceiptsCsv"
                    class="px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 dark:bg-blue-700 dark:hover:bg-blue-800 border border-blue-600 rounded-md transition-all">
                📋 Export CSV
            </button>
        </div>
    </div>
    ...
    @if($receiptStatistics)
        <div class="mb-6 grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div class="bg-orange-50 dark:bg-orange-900/30 border border-orange-200 dark:border-orange-800 p-4 rounded-lg">
                <p class="text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">🛣️ Short Routes</p>
                <p class="text-2xl font-bold text-orange-600 dark:text-orange-400">{{ $receiptStatistics['total_short_route'] ?? 0 }}</p>
            </div>
            <div class="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 p-4 rounded-lg">
                <p class="text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">🛣️ Long Routes</p>
                <p class="text-2xl font-bold text-red-600 dark:text-red-400">{{ $receiptStatistics['total_long_route'] ?? 0 }}</p>
            </div>
        </div>
    @endif
