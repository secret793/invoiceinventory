                ...
                Tables\Actions\Action::make('pickForAffixing')
                    ...
                    ->action(function (ConfirmedAffixed $record, array $data): void {
                        try {
                            ...
                            // Step 1: Create base data shared by all tables
                            $baseData = [
                                'date' => $record->date ?? now(),  // FIX #2: Use actual date if available, not now()
                            ...
                            // Step 2: Create device retrieval record (includes destination_id and user_id)
                            $deviceRetrieval = array_merge($baseData, [
                                ...
                                'user_id' => auth()->id(),  // FIX #1: Capture who is doing the transfer
                                'created_at' => now(),
                                'updated_at' => now(),
                            ]);
                            ...
                            // Step 3: Create monitoring record (does NOT include destination_id or user_id)
                            $monitoringData = array_merge($baseData, [
                                'current_date' => now(),
                                'status' => 'ACTIVE',
                ...
                Tables\Actions\Action::make('returnData')
                    ...
                    ->action(function (ConfirmedAffixed $record, array $data): void {
                        try {
                            ...
                            // Delete assign_to_agents record if exists (using Eloquent to trigger observer)
                            ...
                            foreach ($assignmentsToDelete as $assignment) {
                                // This delete() triggers ReceiptObserver::deleting()
                                // which increments receipt.used column back
                                $assignment->delete();
                            }
