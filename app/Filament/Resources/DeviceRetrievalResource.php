...
class DeviceRetrievalResource extends Resource
{
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->modifyQueryUsing(fn (Builder $query) => $query->with(['destination', 'allocationPoint']))
            ->filters([
    ...
    public static function getPages(): array
    {
        return [
            ...
            // Edit page disabled - users cannot edit device retrieval records by clicking on them
    ...
    public static function getEloquentQuery(): Builder
    {
        ...
        // Exclude archived records from all active views (only show non-archived)
    ...
    protected function getTableQuery(): Builder
    {
        ...
        // Exclude archived records from table view (only show non-archived)
        $query->where('is_archived', false);

        // For Finance Officer, only show devices with overstay_days >= 1
        if ($user?->hasRole('Finance Officer')) {
            $query->where('overstay_days', '>=', 1);
        }
    ...
    public static function getTableActions($record): array
    {
        ...
        // Retrieve Device action - for NOT_RETRIEVED devices without overstay or with paid bills
        if ($record->retrieval_status === 'NOT_RETRIEVED' &&
            ...
            // If device has overstay, ensure bill is paid
            !($record->overstay_days >= 1 && $record->payment_status !== 'PD') &&
            ...
            $actions[] = Tables\Actions\Action::make('retrieveDevice')
                ...
                ->action(function (DeviceRetrieval $record) {
                    // Final validation: Check if device has overstay without payment
                    if ($record->overstay_days >= 1 && $record->payment_status !== 'PD') {
                        \Filament\Notifications\Notification::make()
                            ->danger()
                            ->title('Bill Required Before Retrieval')
                            ->body('Device has ' . $record->overstay_days . ' day(s) overstay. Bill must be generated and marked as Paid before retrieval.')
                            ->send();
                        return;
                    }
        ...
        // Overdue Bills action - for devices with overstay_days >= 1 that aren't paid
        if ($record->overstay_days >= 1 &&
