            ...
            ->schema([
                Forms\Components\Group::make()
                    ->schema([
                        ...
                        Forms\Components\Section::make('Device & Transport Information')
                            ->schema([
                        ...
                        Forms\Components\Section::make('Waiver Information')
                            ->hidden(fn (Invoice $record = null) => !$record?->isWaived())
                            ->schema([
                                Forms\Components\TextInput::make('waived_by')
                                    ->label('Waived By')
                                    ->disabled()
                                    ->formatStateUsing(fn (?string $state, Invoice $record) => $state ? $record->waivedByUser?->name : 'N/A'),

                                Forms\Components\DateTimePicker::make('waived_at')
                                    ->label('Waived At')
                                    ->disabled(),
                                ...
                                Forms\Components\Textarea::make('waiver_reason')
                                    ->label('Waiver Reason')
                                    ->disabled()
                                    ->formatStateUsing(fn (Invoice $record) => $record->waiverRecord?->reason ?? 'N/A'),
                            ])
                            ->columns(2),
                    ])
            ]);
        ...
        return $table
            ...
            ->actions([
                Tables\Actions\Action::make('approve')
                    ...
                    ->visible(fn (Invoice $record) => false), // Hidden - Invoices are auto-approved
                ...
                Tables\Actions\Action::make('reject')
                    ...
                    ->visible(fn (Invoice $record) => false) // Hidden - Cannot reject auto-approved invoices
