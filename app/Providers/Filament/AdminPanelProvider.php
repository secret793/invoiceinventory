            ...
            ->navigation(function (NavigationBuilder $builder): NavigationBuilder {
                ...
                if ($isFinanceOfficer) {
                    $builder->group('Finance Management', [
                        NavigationItem::make('Overstay Receipts')
                            ->icon('heroicon-o-document-text')
                            ->url(InvoiceResource::getUrl())
                            ->badge(
                                fn () => \App\Models\Invoice::where(function ($q) {
                                    $q->where('overstay_days', '>', 0)->orWhere('status', 'WAIVED');
                                })->count() ?: null,
                                color: fn () => 'warning'
                            ),
                       
                        NavigationItem::make('Dispatch Records')
                            ->icon('heroicon-o-truck')
                            ->url(\App\Filament\Resources\DispatchFinanceRecordResource::getUrl())
                            ->badge(
                                fn () => \App\Models\DispatchFinanceRecord::where('status', 'PENDING')->count() ?: null,
                                color: fn () => 'warning'
                            ),
                        NavigationItem::make('Generated Receipts')
                            ->icon('heroicon-o-ticket')
                            ->url(GeneratedReceiptResource::getUrl())
                            ->badge(
                                fn () => \App\Models\Receipt::whereNotNull('generated_by_user')->count() ?: null,
                                color: fn () => 'info'
                            ),
                    ]);
                }
                ...
                if ($isDataEntryOfficer && !$isSuperAdmin && !$isWarehouseManager) {
                    $builder->group('Data Entry/Assignment',
                        DataEntryAssignment::with('allocationPoint')
                            ->where('show_in_menu', true) // Only show assignments marked for menu display
                ...
                if ($isAllocationOfficer && !$isSuperAdmin && !$isWarehouseManager) {
                    ...
                    $allocationPoints = AllocationPoint::all()
                        ->filter(function (AllocationPoint $point) {
                            // Use the safer permission check service
                            $permissionName = 'view_allocationpoint_' . Str::slug($point->name);
                            $hasPermission = PermissionCheckService::userHasPermission(auth()->user(), $permissionName);
