...
class InvoiceObserver
{
    ...
    {
        Log::info('Invoice created event triggered', [
            'invoice_id' => $invoice->id,
            'status' => $invoice->status,
            'device_retrieval_id' => $invoice->device_retrieval_id
        ]);

        // Update DeviceRetrieval payment status when invoice is created
        if ($invoice->device_retrieval_id) {
            try {
                ...
                if ($updated) {
                    Log::info('Successfully updated DeviceRetrieval payment_status on invoice creation', [
                        'invoice_id' => $invoice->id,
                        'device_retrieval_id' => $invoice->device_retrieval_id,
                ...
                Log::error('Error updating DeviceRetrieval on invoice creation', [
                    'invoice_id' => $invoice->id,
                    'device_retrieval_id' => $invoice->device_retrieval_id,
                    'error' => $e->getMessage()
                ]);
