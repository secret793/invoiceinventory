...
class DeviceRetrievalObserver
{
    ...
    public function creating(DeviceRetrieval $deviceRetrieval)
    {
        ...
        // IMPORTANT: Only set destination if it's not already set!
        // This preserves the destination from ConfirmedAffixed or other sources
        if (empty($deviceRetrieval->destination)) {
            // Only use regime-based mapping as a FALLBACK for null/empty values
            ...
            if ($regime) {
                switch (strtolower($regime->name)) {
                    ...
                    case 'transit':
                        ...
                        break;
                    // NOTE: Don't set 'Unknown' as default - leave it null instead
                    // This is safer and easier to identify issues
                }
            }
        }
    ...
    public function updating(DeviceRetrieval $deviceRetrieval)
    {
        try {
            Log::info('DeviceRetrieval updating', [
                ...
                'is_payment_status_changing' => $deviceRetrieval->isDirty('payment_status'),
                'old_status' => $deviceRetrieval->getOriginal('retrieval_status'),
                'new_status' => $deviceRetrieval->retrieval_status
            ]);

            // Handle waiver: Reset overstay fields when payment_status changes to WAIVED
            if ($deviceRetrieval->isDirty('payment_status') && $deviceRetrieval->payment_status === 'WAIVED') {
                Log::info('DeviceRetrieval: Waiver detected - resetting overstay fields', [
                    'id' => $deviceRetrieval->id,
                    'old_payment_status' => $deviceRetrieval->getOriginal('payment_status'),
                    'new_payment_status' => $deviceRetrieval->payment_status,
                    'resetting_overstay_days' => 0,
                    'resetting_overstay_amount' => 0
                ]);
                
                // Reset overstay fields
    ...
    public function retrieved(DeviceRetrieval $deviceRetrieval): void
    {
        try {
            ...
            // IMPORTANT: Only update current_time, DO NOT override destination
            // The destination should be preserved from the original transfer
            // This observer is only responsible for tracking retrieval timestamp

            // Update only the current_time without triggering events to avoid infinite loop
            DeviceRetrieval::withoutEvents(function () use ($deviceRetrieval) {
                $updates = [
                    'current_time' => now(),
                ];
                ...
                Log::debug('DeviceRetrieval current_time updated (destination preserved)', [
                    'device_retrieval_id' => $deviceRetrieval->id,
                    'device_id' => $deviceRetrieval->device_id,
