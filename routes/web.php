...
Route::get('/export/device-retrieval-report-2', [\App\Http\Controllers\DeviceRetrievalReport2Controller::class, 'export'])
    ->name('export.device-retrieval-report-2')
    ->middleware(['auth']);

Route::get('/export/receipts', [\App\Http\Controllers\ReceiptExportController::class, 'export'])
    ->name('export.receipts')
    ->middleware(['auth']);

Route::get('/export/generated-receipts', [\App\Http\Controllers\GeneratedReceiptsExportController::class, 'export'])
...
Route::get('/export/overstay-receipts', [\App\Http\Controllers\OverstayReceiptsExportController::class, 'export'])
    ->name('export.overstay-receipts')
    ->middleware(['auth']);

Route::get('/export/overstay-invoices', [\App\Http\Controllers\Finance\OverstayReceiptsExportController::class, 'export'])
    ->name('export.overstay-invoices')
    ->middleware(['auth']);

Route::get('/export/overstay-devices', [\App\Http\Controllers\OverstayDevicesExportController::class, 'export'])
    ->name('export.overstay-devices')
    ->middleware(['auth']);

// Overstay Devices PDF download route
Route::get('/export/overstay-devices/pdf', function (\Illuminate\Http\Request $request) {
    $filters = $request->query();
    return (new \App\Exports\OverstayInvoicePDFExport($filters))->download();
})->name('export.overstay-devices-pdf')->middleware(['auth']);

// Individual Overstay Invoice PDF route
Route::get('/invoices/{invoice}/pdf', function (\App\Models\Invoice $invoice) {
    return (new \App\Exports\OverstayInvoiceSinglePDFExport($invoice))->stream();
...
Route::get('/receipts/{receipt}/pdf', function (\App\Models\Receipt $receipt) {
    return (new \App\Exports\ReceiptPDFExport($receipt))->stream();
